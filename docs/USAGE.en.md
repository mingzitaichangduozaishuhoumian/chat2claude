...
Access logging is installed only for `/v1/*` and `/admin/api/*`. The default `ACCESS_LOG_FORMAT=text` emits aligned arrows: `[...] <-- METHOD path` at request start and `[...] --> STATUS | duration | METHOD path?query` when the response is ready. SSE is logged at readiness without reading, cloning, teeing, or delaying the body. `detailed` appends allowlisted safe metadata and emits a terminal `--> STREAM` summary; `simple` normalizes to `text`. `json` retains the `{ level, message: "HTTP access", meta, time }` envelope and late stream failures. Logs never contain prompts, tool arguments/results, encrypted content, raw provider payloads, headers, tokens, cookies, session, or proxy credentials.
