  ...
  app.post('/v1/chat/completions', async (c) => {
    try {
      ...
      try {
        ...
        if (request.stream) {
          const events = releaseAccountWhenDone(deps.accountPool, account.id, mapChatGptStreamToOpenAiChatSse(request, deps.backend.stream(backendRequest, backendContext)));
