...
export function renderQuotaCards(quotas: AccountQuotaResult[], accounts: AdminAccountView[] = [], locale: AdminLocale = 'zh-CN'): string {
  ...
  return localizeAdminMarkup(`<div class="quota-grid">${quotas.map((result) => renderQuotaCard(result, accountsById.get(result.accountId))).join('')}</div>`, locale);
}
...
function renderQuotaCard(result: AccountQuotaResult, account: AdminAccountView | undefined): string {
  ...
  return `<article class="quota-card" data-quota-account="${esc(result.accountId)}">
    <header class="card-header"><div class="identity"><span class="eyebrow">${esc(planType)}</span><h3 data-i18n-ignore>${esc(accountLabel)}</h3><p class="wrap-anywhere" data-i18n-ignore>${esc(accountIdentity)}</p></div><span class="state-badge ${state.tone}">${esc(state.label)}</span></header>
    ...
    <footer class="quota-footer"><span>最后获取：${timeHtml(result.fetchedAt)}</span>${result.error ? `<span>状态详情：${esc(result.error.message)} <code>${esc(result.error.code)}</code></span>` : ''}${result.supported ? `<button type="button" class="secondary" data-quota-refresh="${esc(result.accountId)}">刷新此账号</button>` : ''}</footer>
  </article>`;
}
