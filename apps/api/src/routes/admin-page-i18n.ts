...
export const ADMIN_MESSAGES: Record<string, string> = {
  ...
  '名称': 'Name', '安全前缀': 'Safe prefix', '创建时间': 'Created', '操作': 'Actions', '撤销': 'Revoke', '没有运行时 API Key。': 'No Runtime API Keys.', '确认撤销此运行时 API Key？撤销后对应客户端会立即失效。': 'Revoke this Runtime API Key? Clients using it will stop working immediately.', '运行时 API Key 已撤销；ChatGPT 账号授权不会被移除。': 'Runtime API Key revoked; ChatGPT account authorization was not removed.', '运行时 API Key 加载失败，未加载。': 'Runtime API Keys failed to load.',
