...
function fixture(createRoute: typeof createMessagesRoute | typeof createOpenAiChatRoute | typeof createOpenAiResponsesRoute, backend: ChatGptBackendClient, extra: Partial<Parameters<typeof createMessagesRoute>[0] & Parameters<typeof createOpenAiChatRoute>[0] & Parameters<typeof createOpenAiResponsesRoute>[0]> = {}) {
  ...
  const log = logger();
  const app = new Hono();
  // These assertions cover internal lifecycle diagnostics, which human text
  // formats intentionally suppress for actual SSE streams.
  app.use('*', accessLog(log, 'json'));
...
function assertTerminal(f: ReturnType<typeof fixture>, outcome: string, status: number) {
  ...
  const entries = f.log.access.mock.calls.map(([entry]) => entry as { phase?: string; outcome?: string; status: number; lifecycle?: string });
  expect(entries[0]?.phase).toBe('request_started');
  const responseReady = entries.find(entry => entry.phase === 'response_ready');
  expect(responseReady).toMatchObject({ phase: 'response_ready' });
  const terminal = entries.find(entry => entry.phase === 'stream_terminal');
  const lifecycle = entries.filter(entry => entry.phase === 'stream_lifecycle');
  if (terminal) {
    if (lifecycle.length > 0) expect(lifecycle[0]).toMatchObject({ phase: 'stream_lifecycle', lifecycle: 'start' });
    expect(terminal).toMatchObject({ phase: 'stream_terminal', outcome });
    expect(entries.at(-1)).toBe(terminal);
  }
  // A JSON non-stream failure carries terminal diagnostics on response readiness.
  else if (responseReady?.status !== 200) expect(responseReady).toMatchObject({ status, outcome });
...
for (const [createRoute, path, body] of routes) {
  ...
  it(`${path}: explicit unsuccessful done cannot emit successful finish`, async () => {
    ...
    assertTerminal(f, 'failure', 200);
    expect(f.log.access).toHaveBeenCalledWith(expect.objectContaining({ code: 'invalid_response' }), 'json');
  });
  ...
  it.each([
    ...
    assertTerminal(f, 'failure', 200);
    expect(f.log.access).toHaveBeenCalledWith(expect.objectContaining({ code: 'invalid_response', protocolStage: expect.any(String), protocolReason }), 'json');
  });
  ...
  it.each(['cancel', 'error', 'timeout'] as const)(`${path}: %s racing with later cancellation has one terminal`, async winner => {
    ...
    if (winner === 'timeout') expect(f.log.access).toHaveBeenCalledWith(expect.objectContaining({ timeoutKind: 'stream_idle' }), 'json');
  });
  it.each(['response_headers', 'stream_bootstrap'] as const)(`${path}: %s timeout returns JSON 504`, async kind => {
    ...
    expect(f.log.access).toHaveBeenCalledWith(expect.objectContaining({ timeoutKind: kind }), 'json');
  });
}
