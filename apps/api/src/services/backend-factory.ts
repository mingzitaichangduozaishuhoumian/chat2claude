...
export function createChatGptBackend(env: AppEnv, accountPool: AccountPool, durableState?: DurableRuntimeState): ChatGptBackendClient {
  if (env.chatGptBackend === 'session') {
    const transport = new SessionChatGptBackend({ baseUrl: env.chatGptBaseUrl, timeoutMs: env.chatGptRequestTimeoutMs, clientVersion: env.codexClientVersion });
    const oauthClient = new CodexOAuthClient({ timeoutMs: env.chatGptRequestTimeoutMs, clientVersion: env.codexClientVersion });
